...
translator.translatequery = function (query, onDone5) {
	...
	var orderquerywhereclause = function () {//make statements out of the spo and filter and optional and reaggrange them or this can be done at object level, then at this level only inter object needs to be catered
	};
	...
	var getabstract = function (id) {
		...
		globalquery.whereclause.filter.push("FILTER(langMatches(lang(?description), \"en\")) ");
		globalquery.whereclause.filter.push("FILTER(langMatches(lang(?name), \"en\")) ");
	...
	var parseobject = function (mqlpointer, sparqlparent, id, resultpath) {
		...
		if (findtype(queryobject["id"]) == 7) { //id="<"+queryobject["id"]+">";
	...
	for (var i in globalquery.selectclause) { finalquery += " " + globalquery.selectclause[i]; }
	finalquery += "  where {";
	for (var i in globalquery.whereclause.s) {
		for (var j in substitute)
			if (globalquery.whereclause.s[i] == substitute[j].id)
	...
	for (var i in globalquery.whereclause.filter) {
		finalquery += globalquery.whereclause.filter[i];
	}
